...
TEXT_FONT_PATH = "/home/pi/.fonts/Uah_unfontpossibile-Regular (1).ttf"   # Percorso Linux. Copia il font qui o imposta None per usare font OpenCV
...
def send_arduino_frame(ser, frame, use_gamma=True):
    ...
    # Sovrappone la scritta anche sulla matrice Arduino
...
def main():
    ...
    disp_mode = "ESP" if ESP_ENABLED else "Arduino"
    if ESP_ENABLED and ARDUINO_ENABLED: disp_mode = "DUAL (ESP + Arduino)"
    
    print(f"\n[STREAM] Inviando streaming video modalità {disp_mode}...")
    ...
    try:
        while True:
            ...
            # Sovrappone la scritta (Specchio) con contrasto automatico
            apply_text_overlay(frame_ridimensionato, TEXT_OVERLAY)
        ...
        if arduino_ser:
            try:
                ...
                # Invia frame nero all'Arduino usando il nuovo header
